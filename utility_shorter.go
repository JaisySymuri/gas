...
func EnvInit() {
	config := viper.New()
	config.SetConfigFile("appd_config.env")
	config.AddConfigPath(".")

	err := config.ReadInConfig()
	if err != nil {
		fmt.Errorf("EnvInit function failed to read config file: %s, error: %s", "appd_config.env", err)
	}

	kkhost := config.GetString("HOST")
	kkport := config.GetString("PORT")
	kkversion := config.GetString("VERSION")
	privateKey := config.GetString("PRIVATE_KEY")
	certificate := config.GetString("CERTIFICATE")
}
